...
const fs = require("fs");
const DATA = JSON.parse(fs.readFileSync("E:/github/Dividend-Strategy-Lab/output/rotation.json", "utf-8"));
const R = computeAll(DATA);
console.log("dates:", R.dates[0], "~", R.dates[R.dates.length-1], "n=", R.dates.length);
console.log("turns:", R.turns, " curW:", R.curW, " curPct:", R.curPct, " curDiff:", R.curDiff);
console.log("ann:", (R.ann*100).toFixed(2)+"%", " annD:", (R.annD*100).toFixed(2)+"%", " excess:", (R.excess*100).toFixed(2)+"%");
console.log("mdd:", (R.mdd*100).toFixed(2)+"%", " sharpe:", R.sharpe.toFixed(2), " winRate:", R.winRate);
console.log("lastSig:", R.lastSig, " holdDays:", R.holdDays);
for (const s of R.switches) console.log(s.date, (s.from*100)+"%->"+(s.to*100)+"%", s.pct!=null?s.pct.toFixed(2):"--", s.reason);
